jQuery(document).ready(function($) {
    ...
    function checkAllRequiredFieldsFilled() {
        ...
        return !!(personaId && avatarId && voiceId && systemPrompt);
    }
    
    function updateAllStates() {
        const isVerified = checkApiVerified();
        const allFieldsFilled = checkAllRequiredFieldsFilled();
        const apiKey = $('#anam-api-key').val().trim();
        
        console.log('State check:', { isVerified, allFieldsFilled, apiKeyLength: apiKey.length });
        
        // Verify button: enabled if API key has 1+ characters
        $('#verify-api-key').prop('disabled', apiKey.length === 0);
        
        // Reset All button: enabled if API is verified
        $('#anam-reset-all').prop('disabled', !isVerified);
        
        // Save Settings button: enabled if all required fields filled
        $('#anam-save-settings').prop('disabled', !allFieldsFilled);
        
        // Tabs: enabled if all required fields filled
        const displayTab = $('a[href="?page=anam-settings&tab=display"]');
        const supabaseTab = $('a[href="?page=anam-settings&tab=supabase"]');
        
        if (allFieldsFilled) {
            displayTab.css({ 'pointer-events': 'auto', 'opacity': '1' });
            supabaseTab.css({ 'pointer-events': 'auto', 'opacity': '1' });
        } else {
            displayTab.css({ 'pointer-events': 'none', 'opacity': '0.5' });
            supabaseTab.css({ 'pointer-events': 'none', 'opacity': '0.5' });
        }
        
        // Fields: enabled if API is verified
        if (isVerified) {
    ...
    function autoVerifyApiKey() {
        ...
        if (apiKey && apiKey.trim() !== '' && !isAlreadyVerified) {
            ...
            // API key is already verified - just enable fields and update states
            $('.anam-dependent-field').prop('disabled', false).css('opacity', '1');
            $('#anam-avatar-position').prop('disabled', false).css('opacity', '1');
            $('#page-selection-section input[type="checkbox"]').prop('disabled', false).css('opacity', '1');
            updateAllStates();
    ...
    // Supabase enable checkbox
    $('#supabase-enable').on('change', function() {
    ...
    // API Key input - update states
    $('#anam-api-key').on('input', function() {
        updateAllStates();
    });
    
    // Required fields - update states when they change
    $('input[name="anam_options[persona_id]"], input[name="anam_options[avatar_id]"], input[name="anam_options[voice_id]"], textarea[name="anam_options[system_prompt]"]').on('input change', function() {
        updateAllStates();
    });
