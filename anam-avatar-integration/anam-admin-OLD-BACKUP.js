jQuery(document).ready(function($) {
    ...
    // Simple flag to track if form has been modified
    let formHasChanged = false;
    
    // Function to enable/disable Save Settings button
    function updateSaveButtonState() {
        const isVerified = $('#api-status').text().includes('✅');
        
        // Enable if: API verified AND form has changed
        const shouldEnable = isVerified && formHasChanged;
        
        $('#anam-save-settings').prop('disabled', !shouldEnable);
    ...
    // Reset the changed flag (after save or reset)
    function resetFormChangedFlag() {
        formHasChanged = false;
        updateSaveButtonState();
        console.log('Form change flag reset');
    }
            ...
            success: function(response) {
                ...
                setTimeout(function() {
                    ...
                    if (response.success) {
                        ...
                        // Reset form changed flag after successful verification
    ...
    function autoVerifyApiKey() {
        ...
        if (apiKey && apiKey.trim() !== '' && !isAlreadyVerified) {
            ...
            // API key is already verified - just enable fields and reset flag
            $('.anam-dependent-field').prop('disabled', false).css('opacity', '1');
            $('#anam-avatar-position').prop('disabled', false).css('opacity', '1');
            $('#page-selection-section input[type="checkbox"]').prop('disabled', false).css('opacity', '1');
            $('#anam-reset-all').prop('disabled', false);
    ...
    $('#anam-settings-form').on('input change keyup', ':input', function() {
        const fieldName = $(this).attr('name') || 'unknown';
        console.log('Field changed:', fieldName);
        markFormAsChanged();
    });
    
    // Also listen directly on textareas (belt and suspenders)
    $('textarea').on('input keyup paste', function() {
        console.log('Textarea changed directly');
        markFormAsChanged();
    });
    ...
    // After successful form submission, reset the flag
    $('#anam-settings-form').on('submit', function() {
        console.log('Form submitted - will reset flag after save');
